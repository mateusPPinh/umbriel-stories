...
const FeaturedGrid: React.FC<FeaturedGridProps> = ({ variant, isDarkTheme, customStyles }) => {
    ...
    >
      {Object.entries(articles).map(([colKey, colArticles]) => (
        colArticles.map((article, index) => (
          <div 
            key={article.id}
            className={`flex flex-col gap-4 ${index === 0 ? 'row-span-2' : ''}`}
            style={columnStyle(colKey)}
          >
            <div className={`relative ${index === 0 ? 'aspect-[4/5]' : 'aspect-video'} rounded-lg overflow-hidden`}>
              <Image
            ...
            <div>
              <h3 
                className={`${index === 0 ? 'text-2xl' : 'text-lg'} font-medium mb-2`}
                style={headingStyle}
              >
                {article.title}
              </h3>
