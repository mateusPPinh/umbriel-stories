...
export default function Modal ({ modalContent, modalSubtitle, modalTitle, useCustomCloseButton, dialogTriggerChild }: ModalProps): ReactElement {
  return (
    <Dialog>
    <DialogTrigger>{dialogTriggerChild}</DialogTrigger>
    <DialogContent className='sm:max-w-[480px] space-y-6 px-6 py-6' showCloseButton={useCustomCloseButton}>
      <DialogHeader className="flex items-start justify-between">
        <div className='mb-4'>
          <DialogTitle className='text-[16px] font-medium leading-[110%]'>{modalTitle}</DialogTitle>
          <DialogDescription>
            {modalSubtitle}
          </DialogDescription>
        </div>
